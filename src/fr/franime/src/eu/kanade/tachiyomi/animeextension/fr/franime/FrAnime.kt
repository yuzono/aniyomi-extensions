...
class FrAnime : AnimeHttpSource() {
    ...
    override suspend fun getEpisodeList(anime: SAnime): List<SEpisode> {
        ...
        val episodes = animeData.seasons.getOrNull(season - 1)?.episodes
            ?.mapIndexedNotNull { index, episode ->
                ...
                SEpisode.create().apply {
                    ...
                    name = episode.title ?: "Épisode ${index + 1}"
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        return players.withIndex().parallelCatchingFlatMap { (index, playerName) ->
            ...
            extractedVideos.map { video ->
                Video(video.url, "${video.quality} ($playerName)", video.videoUrl, headers = headers)
            }
        }
    }
